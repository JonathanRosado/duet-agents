...
You each run in your own visible tmux pane and collaborate on the user's task as
peers — think independently, push back, add what Claude misses, do real work.
...
## Sending messages
To reply to Claude, run this (your message body on stdin via the heredoc):
...
Claude is mid-task, add `--interrupt` after `claude`; use it only to redirect urgently.
