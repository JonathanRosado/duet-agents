...
is surfaced as dead or blocked — never silent limbo. Every supported harness
queues input while it is generating, so a peer being busy is never a reason a
message cannot be sent; an unreadable composer is retried, not fenced. On
macOS/Linux a blocked peer can be returned to the session with
`duet-resume.sh <name>` once its pane is alive and idle, and delivery continues
from its queue head without ever pasting a landed message twice. On Windows a
blocked recipient is still terminal; re-init. If a peer seems unresponsive,
check status.
...
To return a blocked peer to the session (macOS/Linux only):
